const words = ["LOCRO", "EMPANADAS", "ASADO", "GUITARRA", "QUENA", "PONCHO", "ACORDEÓN", "CHACARERA", "ZAMBA", "CHAMAMÉ", "TRUCO", "TABA", "BOINA", "ALPARGATAS", "BAGUALA", "CARNAVALITO", "RASTRA", "MILONGA", "BOMBO", "CHIRIPÁ", "MATE", "HUMITA", "SIKU", "CUECA", "CHIPÁ"];
...
let gameEnded = false;   // 🔹 Nuevo flag global
...
function checkCorrectCellsSelected() {
    if (gameSaved || gameEnded) return; // Si el juego ya terminó, no hacer nada
    ...
    if (allCorrectSelected && !anyIncorrectSelected) {
        // Si es la última palabra y bonus aún no aplicado
        if (usedWords.length === totalWords && !bonusApplied) {
            gameEnded = true;       // 🔹 Marcamos juego como terminado
            bonusApplied = true;

            clearInterval(timerInterval); // detener reloj
            disableClicks();              // deshabilitar clics

            // Sumar score de la palabra actual
            score += 30;
            document.getElementById("puntaje").textContent = `Puntaje: ${score}`;
        ...
        // Caso normal: palabra encontrada
        showFoundMessage();
        return;
    }
    ...
    if (totalTimeInSeconds <= 0 && !gameEnded) {
        gameEnded = true;           // 🔹 Evitamos superposición de finales
        updateAcumulado(score);
        showEndGameMessage();
        clearInterval(timerInterval);
    }
